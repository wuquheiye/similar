...
public class PersonUserPositionController {
    ...
    public ReturnMsg save(Integer id,String email) {
        ReturnMsg msg = new ReturnMsg();
        if (id == null) {
            msg.setStatus("203");
            msg.setStatusMsg("id不能为空");
        ...
        try {
            msg = iPersonUserPositionService.informInterview(id, 2,email);
        } catch (Exception e) {
            e.printStackTrace();
            msg.setStatus("201");
            msg.setStatusMsg("更改异常");
    ...
    public ReturnMsg save(String email, int companyPositionId) {
        ReturnMsg msg = new ReturnMsg();
        try {
            msg = iPersonUserPositionService.save(email, companyPositionId);
        } catch (Exception e) {
            e.printStackTrace();
            msg.setStatus("201");
