...
public class PersonUserServiceImpl extends ServiceImpl<PersonUserMapper, PersonUser> implements IPersonUserService {
    ...
    @Override
    public ReturnMsgPage selectPersonUserByCondition(PersonUser personUserCondition, int page, int pageSize) {
        int pageStart = (page - 1) * pageSize;
        ReturnMsgPage msg = new ReturnMsgPage();
        List<PersonUser> personUserList = personUserMapper.selectPersonUserByCondition(personUserCondition,pageStart,pageSize);
        List<PersonVO> personVOList = new ArrayList<>();
        for(PersonUser personUser:personUserList){
            // 获取教育经验
            PersonEducationExperience personEducationExperience = personEducationExperienceMapper.selectByPersonUserId(personUser.getId());
    ...
    @Override
    public int selectCount(PersonUser personUser) {
        return personUserMapper.selectCount(personUser);
    }
}
